...
from google import genai  # الحزمة الحديثة الرسمية
import PIL.Image
import pandas as pd
...
# تهيئة العميل الجديد لـ Gemini
client = genai.Client(api_key=GEMINI_API_KEY)

# إرشادات النظام للوكيل
...
if choice == "المنصة الرئيسية 🏡":
    ...
    with col3:
        st.metric("محرك الذكاء", "Gemini 2.5 / 1.5")
    ...
    if prompt := st.chat_input("اطلب كود كاميرا، استشارة ديكور، أو بحثاً عن صفقة..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("المهندس السيادي يفكر..."):
                try:
                    # طريقة التوليد باستخدام العميل الجديد
                    response = client.models.generate_content(
                        model='gemini-2.5-flash', # أو gemini-1.5-pro حسب رغبتك
                        contents=prompt,
                        config={
        ...
        if st.button("تحليل الورش ميدانياً 🔍"):
            with st.spinner("المهندس السيادي يحلل الصورة..."):
                try:
                    ...
                    # رفع الصورة والتقرير إلى Supabase
                    if project_name:
...
# --- 5. إدارة الصفقات ---
    ...
    with tab1:
        with st.form("deal_form", clear_on_submit=True):
            ...
            with col2:
                client_name = st.text_input("اسم العميل / الجهة:")
