...
# تنظيم أزرار التفضيلات الجانبية بشكل أفقي أو متجاور إن امكن، أو تبسيطها
...
# ==========================================
# 2. الاستوديو البصري والوكيل الذكي للكاميرات (Agentic Super Multi-Domain AI)
# ==========================================
  ...
  # تحويل الإعدادات والخيارات لتكون أفقية ومتجاورة (Horizontal Layout)
  ...
  # أزرار أفقية لتنفيذ المهام البصرية
    ...
    if ai_client:
      try:
        sys_prompt = f"""
                ...
                """
        with st.spinner("جاري صياغة البرومبت الهندسي البصري المحترف..."):
  ...
  if uploaded_file and process_image_btn:
    ...
    # تطبيق إعدادات تحسين تعتمد على الكاميرا المختارة
    ...
    if add_watermark:
      ...
      draw.text((width - 350, height - 40), watermark_text, fill=(255, 255, 255))
        ...
        if ai_client:
          try:
            ...
            with st.spinner("جاري البحث العميق واستخراج البيانات..."):
              response = ai_client.models.generate_content(
                  model="gemini-2.5-flash", contents=research_prompt
              )
  ...
  with tab1:
    with st.form("prop_form"):
      ...
      with col_p2:
        ...
        use_ai = st.checkbox(
            "🤖 توليد وصف تسويقي احترافي باستخدام Gemini AI", value=True
        )
...
# ==========================================
  ...
  with col_s2:
    if ai_client:
      st.success("🤖 تم ربط وكلاء الذكاء الاصطناعي بنجاح (Gemini API).")
