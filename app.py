...
st.sidebar.markdown("---")
st.sidebar.info(f"📍 {agent.commercial_name}\n\nإدارة العمليات بقلعة السراغنة ومراكش.")
...
if selected_tab == "🤖 الوكيل الذكي (الدردشة)":
    ...
    if prompt := st.chat_input("اطرح سؤالك أو استفسارك هنا (عقارات، أراضي فلاحية، أعمال)..."):
        ...
        with st.chat_message("assistant"):
            if "فلاح" in prompt or "أرض" in prompt or "بقعة" in prompt:
                response = "نوفر عروضاً للأراضي الفلاحية والبقع الاستثمارية بقلعة السراغنة ومحيطها بمعايير دقيقة."
            elif "شحن" in prompt or "لوجستيك" in prompt:
                response = "نتابع مسارات الشحن واللوجستيك وسلاسل الإمداد بدقة تامة."
            else:
                response = f"تم استلام طلبك بنجاح في {agent.commercial_name}. نعمل على توفير أفضل الخدمات العقارية والتجارية لك."
...
# --- القسم الثالث: شاشة تحميل وتوثيق الصور ---
    ...
    st.markdown("قم برفع الصور الخاصة بالعقارات، الأراضي، أو التوثيق الميداني لتخزينها ومعالجتها:")
    st.markdown("---")

    uploaded_file = st.file_uploader("اختر صورة للرفع (PNG, JPG, JPEG):", type=["png", "jpg", "jpeg"])
    ...
    if uploaded_file is not None:
        ...
        st.write("📌 **حالة التوثيق:** جاهزة للأرشفة والربط بملفات العقار أو التنسيق الميداني.")

# --- القسم الرابع: رابط التواصل مع الواتساب ---
elif selected_tab == "💬 التواصل المباشر (واتساب)":
    st.title("💬 خدمة العملاء والتواصل المباشر")
    st.markdown("للتواصل الفوري مع إدارة المكتب أو طلب استشارة عقارية وتجارية عاجلة:")
    st.markdown("---")
    
    whatsapp_number = "212691897126"
    whatsapp_text = "مرحباً، أهلاً بك في سراغنة العقارية. أود الاستفسار عن العروض المتاحة."
