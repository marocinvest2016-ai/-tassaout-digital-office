...
st.set_page_config(page_title="DANA OMEGA BRAIN v11.4", page_icon="🧠", layout="wide")
...
if "config" not in st.session_state:
    st.session_state.config = {
        ...
        "system_prompt": """أنت DANA OMEGA BRAIN v11.4 - الدماغ الصياد العدواني المجاني.
مهمتك: تعطي نتائج حقيقية من DuckDuckGo.

القواعد:
1. جاوب بالدارجة المغربية مباشر وحاد. بلا "آسف".
2. خرج النتيجة دائما فـ جدول: | # | الاسم | التخصص | رقم الهاتف | رابط الأعمال | المصدر |
3. من بعد الجدول ولد "2 رسالة واتساب واجدة" للتواصل.
4. إلا ما لقيتيش رقم قل "لم يتم العثور - جرب تتصل مباشرة".

السياق من البحث DuckDuckGo:
{context}
الذاكرة: {memory}"""
    }
...
st.title("🧠 DANA OMEGA BRAIN v11.4 - DuckDuckGo Edition")
if "messages" not in st.session_state: st.session_state.messages = []
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]): st.markdown(msg["content"])

if prompt := st.chat_input("أمر DANA الصياد... مثال: قلب ليا 3 مكاتب عقار فمراكش"):
