...
with c_side1:
    privacy_mode = st.checkbox("🔒 نمط الخصوصية", value=False)
with c_side2:
    dark_mode_ui = st.checkbox("🌙 الوضع الليلي", value=True)
...
menu = st.sidebar.radio(
    ...
    [
        ...
        "📸 الاستوديو البصري والوكيل الذكي للكاميرات",
...
if menu == "📊 لوحة القيادة والمؤشرات الشاملة":
    ...
    try:
        ...
        if privacy_mode:
            ...
            col4.metric(
                "إجمالي قيمة الصفقات (درهم)",
                f"{total_revenue:,.2f} درهم مغربي",
            )
...
# ==========================================
# 2. الاستوديو البصري والوكيل الذكي للكاميرات
# ==========================================
elif menu == "📸 الاستوديو البصري والوكيل الذكي للكاميرات":
    st.title(
        "👑 الوكيل الذكي متعدد المجالات للكاميرات والهويات البصرية (Agentic"
        " Super Multi-Domain AI)"
    )
    st.write(
        "هندسة وتوليد الهويات البصرية، تحويل البرومبتات، وإدارة محاكاة العدسات"
        " والكاميرات الاحترافية بدقة فائقة."
    )

    col_opt1, col_opt2, col_opt3 = st.columns(3)
    with col_opt1:
        camera_model = st.selectbox(
            "محاكاة محرك الكاميرا:",
            [
                "Hasselblad X2D 100C (عقارات فاخرة وفيلات)",
                "Sony A1 (أراضي وتجزئات قلعة السراغنة)",
                "Canon EOS R5 (توثيق ميداني وإعلانات تسويقية)",
                "Leica M11 (هويات بصرية فخمة وموروكوستايل)",
            ],
        )
    with col_opt2:
        visual_style = st.selectbox(
            "طابع الهوية البصرية:",
            [
                "سينمائي احترافي (Cinematic Luxury)",
                "معماري هندسي نقي (Architectural Clean)",
                "تسويقي تجاري جذاب (Commercial Vibrant)",
            ],
        )
    with col_opt3:
        add_watermark = st.checkbox(
            "إضافة علامة مائية تلقائية (Tassaout)", value=True
        )

    st.markdown("---")
    user_prompt_vision = st.text_area(
        "أدخل البرومبت أو الوصف النصي للمشروع البصري أو العقاري:"
    )

    col_btn1, col_btn2 = st.columns(2)
    with col_btn1:
        generate_ai_prompt_btn = st.button(
            "🤖 هندسة وتحسين البرومبت بالذكاء الاصطناعي", use_container_width=True
        )
    with col_btn2:
        process_image_btn = st.button(
            "✨ معالجة وتطبيق الفلاتر السيادية على الصورة", use_container_width=True
        )
    ...
    if generate_ai_prompt_btn and user_prompt_vision:
        if ai_model:
            try:
                ...
                بناءً على الكاميرا المختارة '{camera_model}' والطابع '{visual_style}'، 
                قم بتحويل هذا الوصف إلى برومبت احترافي فائق الدقة باللغة الإنجليزية لتوليد الصور والتصميمات الهندسية: '{user_prompt_vision}'.
                """
                with st.spinner(
                ...
                st.error(f"خطأ في توليد البرومبت: {e}")
        else:
            st.info("محاكاة: تم توليد البرومبت البصري بنجاح.")

    uploaded_file = st.file_uploader(
        "أو ارفع صورة حقيقية لتطبيق معالجة الكاميرا الاحترافية:",
        type=["jpg", "jpeg", "png"],
    )

    if uploaded_file and process_image_btn:
        image = Image.open(uploaded_file).convert("RGB")
        st.image(image, caption="الصورة الأصلية", use_container_width=True)

        sharp_factor = 2.2 if "Hasselblad" in camera_model else 1.8
        contrast_factor = 1.7 if "Sony" in camera_model else 1.5

        enhancer = ImageEnhance.Sharpness(image)
        img_enhanced = enhancer.enhance(sharp_factor)
        ...
        if add_watermark:
            ...
            watermark_text = "Tassaout Immobiliere - Sraghna / Marrakech"
            draw.text(
                (width - 350, height - 40), watermark_text, fill=(255, 255, 255)
            )

        st.image(
            img_final,
            caption=(
                "الصورة بعد المعالجة البصرية بواسطة عدسة محاكاتنا السيادية"
            ),
            use_container_width=True,
        )
        file_path = os.path.join(
            GALLERY_FOLDER,
            f"studio_processed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg",
        )
        img_final.save(file_path)
        st.success(f"تم حفظ العمل البصري في الأرشيف بنجاح: {file_path}")

# ==========================================
# 3. غرفة قيادة وكلاء الذكاء الاصطناعي
# ==========================================
    ...
    with st.form("agent_command_form"):
        ...
        submit_agent = st.form_submit_button(
            "🚀 إرسال المهمة للوكيل الذكي", use_container_width=True
        )
        ...
        if submit_agent:
            if user_task and ai_model:
                try:
                    system_personas = {
                        "🏢 وكيل العقارات وتحليل السوق المغربي": (
                            "أنت خبير استراتيجي عقاري في السوق المغربي (قلعة"
                            " السراغنة ومراكش)."
                        ),
                        "🤝 وكيل المبيعات وإغلاق الصفقات (CRM Expert)": (
                        ...
                        "📢 وكيل التسويق الرقمي والحملات (Sraghna Media / DANA)": (
                            "أنت مدير تسويق رقمي وإعلانات ممولة مستهدف للسوق"
                            " المغربي."
                        ),
                        "💰 وكيل الإدارة المالية وتقييم الأرباح (CFO Agent)": (
                            "أنت مستشار مالي محترف لإدارة العائد على الاستثمار"
                            " ROI."
                        ),
                    }
                    prompt = (
                        f"{system_personas[agent_type]}\n\nالمهمة: {user_task}"
                    )
                    with st.spinner(
    ...
    with st.form("deep_research_form"):
        ...
        with col_rs1:
            research_query = st.text_input(
                "ما الذي تريد من وكيل البحث العميق استخراجه؟"
            )
        with col_rs2:
        ...
        run_research = st.form_submit_button(
            "🔍 بدء البحث العميق وتحليل الداتا", use_container_width=True
        )
            ...
            if research_query:
                if ai_model:
                    try:
                        ...
                        st.error(
                            f"فشل تشغيل وكيل البحث العميق: {rs_err}"
                        )
                else:
                    st.success(
                        "✅ تم إنجاز البحث العميق (محاكاة محلية بنجاح)."
                    )
            else:
    ...
    with tab1:
        with st.form("prop_form"):
            ...
            with col_p2:
                ...
                use_ai = st.checkbox(
                    "🤖 توليد وصف تسويقي احترافي باستخدام Gemini AI",
                    value=True,
                )
    ...
    with st.form("crm_form"):
        ...
        with col_c2:
            ...
            lead_quality = st.selectbox(
                "مستوى الجودة",
                ["🔥 عميل ساخن", "⚡ عميل مهتم", "🧊 عميل بارد"],
            )

        if st.form_submit_button("حفظ العميل", use_container_width=True):
            if full_name:
                payload = {
                    "full_name": full_name,
                    "phone": phone,
    ...
    try:
        ...
        if c_list:
            ...
            if "phone" in df_c.columns:
                ...
                sel_p = st.selectbox(
                    "اختر هاتف العميل لمراسلة واتساب:", phones
                )
                if sel_p:
    ...
    with st.form("deal_form"):
        ...
        with col_d1:
            contact_id = st.number_input(
                "معرف العميل (Contact ID)", min_value=1, step=1
            )
            amount = st.number_input("مبلغ الصفقة الإجمالي (درهم)", step=1000.0)
        with col_d2:
            ad_expense = st.number_input(
                "تكلفة الحملة الإعلانية (درهم)", value=0.0
            )
            stage = st.selectbox(
                "المرحلة",
                [
        ...
        if not privacy_mode:
            st.info(
                f"العمولة (3%): {comm:,.2f} د.م | صافي الربح:"
                f" {net_profit:,.2f} د.م"
            )

        if st.form_submit_button("حفظ الصفقة", use_container_width=True):
            payload = {
...
# ==========================================
    ...
    with t1:
        ...
        if st.button("حساب القسط البنكي", use_container_width=True):
            loan = p - down
            m_rate = (rate / 100) / 12
            m_pay = (
                loan
                * (m_rate * (1 + m_rate) ** (yrs * 12))
                / ((1 + m_rate) ** (yrs * 12) - 1)
            )
    ...
    with t2:
        ...
        with col_rc1:
            inv = st.number_input("تكلفة الاستثمار الشاملة (درهم)", value=600000.0)
        with col_rc2:
            m_rent = st.number_input(
                "الإيجار الشهري المتوقع (درهم)", value=4000.0
            )

        if st.button("حساب عائد الاستثمار (ROI)", use_container_width=True):
...
# ==========================================
    ...
    with col_lg1:
        car = st.selectbox(
            "السيارة:", ["Dacia Duster", "Renault Clio 5", "Hyundai Tucson"]
        )
    with col_lg2:
    ...
    if st.button("حساب التكلفة وإنشاء رابط الحجز", use_container_width=True):
        tot = days * rate
...
# ==========================================
    ...
    if files and st.button(
        "تنزيل الأرشيف البصري (ZIP)", use_container_width=True
    ):
                ...
                for f in filenames:
                    zipf.write(
                        ...
                        arcname=os.path.relpath(
                            os.path.join(root, f), GALLERY_FOLDER
                        ),
                    )
        with open(zip_path, "rb") as f:
            st.download_button(
                "تحميل الأرشيف المضغوط الآن",
                f,
