...
def call_meta_ai(prompt, agent_name):
    """إرسال الطلب بالطريقة الصحيحة عبر POST لتجنب خطأ المسار أو نوع الطلب"""
    ...
    try:
        # التأكد من إرسال الطلب حصرياً عبر POST وبدون أي إعادة توجيه خاطئة
        res = requests.post(url, headers=headers, json=payload, timeout=60, allow_redirects=False)
        res.raise_for_status()
        return res.json()['choices'][0]['message']['content']
    except requests.exceptions.HTTPError as err:
        return f"❌ خطأ من الخادم: {err}. استجابة الخادم: {res.text}"
